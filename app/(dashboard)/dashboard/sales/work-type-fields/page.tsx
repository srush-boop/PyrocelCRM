...
import type { Profile, WorkTypeField, SystemType } from '@/lib/types/database'
...
export default async function WorkTypeFieldsPage() {
  ...
  const [{ data: fields }, { data: systemTypes }] = await Promise.all([
    supabase.from('work_type_fields').select('*').order('work_type').order('position'),
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">
          Define the conditional &quot;IF&quot; questions that appear on a system based on its
          system type and type of work. For example, a Fire Alarm install might ask for cable type;
          supply only asks nothing.
        </p>
      </div>
      <WorkTypeFieldsManager
        fields={(fields ?? []) as WorkTypeField[]}
