// Pure, client-safe builder for the PO-request email.
// No server-only imports live here so the same template can render a live
// preview in the review dialog AND be sent from the server action.

export interface PoRequestEmailContent {
  siteName: string
...
export function poRequestEmailSubject(options: PoRequestEmailContent): string {
  return `PO Request — ${options.serviceName} at ${options.siteName}`
}

export function buildPoRequestEmailHtml(options: PoRequestEmailContent): string {
    ...
    options.parts.length > 0
      ? options.parts
          .map(
            (p) =>
              `<tr>
                <td style="padding:4px 12px 4px 0;border-bottom:1px solid #eee;">${esc(p.name)}</td>
            ...
            .map(
              (r) =>
                `<li style="margin-bottom:4px;">
                  Requested ${formatDate(r.created_at)} by ${esc(r.requester?.full_name || r.requester?.email || 'staff')}
                  ${r.note ? ` — <em>${esc(r.note)}</em>` : ''}
                  ${r.po_number ? ` <strong>| PO: ${esc(r.po_number)}</strong>` : ''}
  ...
  const specialNoteBlock = options.specialNote
    ? `<div style="margin:20px 0;padding:14px 16px;background:#eff6ff;border:1px solid #93c5fd;border-radius:6px;">
        <p style="margin:0;font-size:14px;color:#1e40af;"><strong>Note from ${esc(options.companyName)}:</strong> ${esc(options.specialNote)}</p>
      </div>`
    : ''
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08);max-width:600px;width:100%;">
        ...
        <tr><td style="background:#18181b;padding:20px 32px;">
          <p style="margin:0;color:#fff;font-size:18px;font-weight:700;">${esc(options.companyName)}</p>
          <p style="margin:4px 0 0;color:#a1a1aa;font-size:13px;">Purchase Order Request</p>
        </td></tr>

        <!-- Body -->
        <tr><td style="padding:28px 32px;">
          ${options.contactName ? `<p style="margin:0 0 16px;font-size:15px;color:#333;">Dear ${esc(options.contactName)},</p>` : ''}
          <p style="margin:0 0 20px;font-size:15px;color:#333;">
            We have completed work at <strong>${esc(options.siteName)}</strong>${options.clientName ? ` for <strong>${esc(options.clientName)}</strong>` : ''} and kindly request a Purchase Order number to proceed with invoicing.
          </p>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:20px;background:#f9f9f9;border:1px solid #e5e5e5;border-radius:6px;overflow:hidden;">
            ...
            <tr><td style="padding:12px 16px;">
              <table width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;">
                <tr><td style="color:#666;padding:3px 0;width:40%;">Service:</td><td style="font-weight:500;">${esc(options.serviceName)}</td></tr>
                ${options.systemName ? `<tr><td style="color:#666;padding:3px 0;">System:</td><td style="font-weight:500;">${esc(options.systemName)}</td></tr>` : ''}
                ${options.panelName ? `<tr><td style="color:#666;padding:3px 0;">Panel(s):</td><td style="font-weight:500;">${esc(options.panelName)}</td></tr>` : ''}
                ${options.referenceNumber ? `<tr><td style="color:#666;padding:3px 0;">Call Reference:</td><td style="font-weight:500;">${esc(options.referenceNumber)}</td></tr>` : ''}
                <tr><td style="color:#666;padding:3px 0;">Completed:</td><td style="font-weight:500;">${formatDate(options.completedAt)}</td></tr>
                ${options.clientRef ? `<tr><td style="color:#666;padding:3px 0;">Client Ref:</td><td style="font-weight:500;">${esc(options.clientRef)}</td></tr>` : ''}
          ...
          <p style="margin:0;font-size:14px;color:#666;">
            ...
            <strong>${esc(options.companyName)}</strong>
          </p>
        </td></tr>

        <!-- Footer -->
        <tr><td style="padding:16px 32px;background:#f3f4f6;border-top:1px solid #e5e5e5;">
          <p style="margin:0;font-size:12px;color:#888;">This email was sent by ${esc(options.companyName)}. You are receiving this because your details are registered with this site.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
