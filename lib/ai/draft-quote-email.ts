...
// Model used for drafting. A small, fast model is plenty for short business
// emails and keeps latency + cost low. Uses the Vercel AI Gateway (zero-config
// via AI_GATEWAY_API_KEY / OIDC), so no provider package is required.
const DRAFT_MODEL = 'openai/gpt-5.4-mini'
...
export interface DraftQuoteEmailResult {
  ok: boolean
  subject?: string
  body?: string
  error?: string
