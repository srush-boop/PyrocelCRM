...
const { data, error } = await sb.rpc('exec_sql', { sql: "select 1" }).catch(e=>({error:e}))
// Fallback: query pg_policies via PostgREST not possible; use a direct SQL through rpc not available.
// Instead inspect columns/data for tasks and completed task_results.
